...
export const CreateArticleForm = () => {
  ...
  const onSubmit = async (values: ArticleFormData) => {
    const { error, data } = await createArticleAction(
  ...
  const handleReset = () => {
    form.reset();
    setEditorKey((prev) => (prev === 0 ? prev + 1 : prev - 1));
  };
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Create article, adding various blocks of different types.
        </CardDescription>
      </CardHeader>
      <CardContent>
                ...
                return (
                  <Field data-invalid={fieldState.invalid}>
                    <Suspense fallback={<Spinner className="size-12" />}>
                      <Editor key={editorKey} onChange={field.onChange} />
                    </Suspense>
